...
SET 
  seed_planting_action = (
    CASE 
      WHEN (wp.data->>'seedPlanting') IS NOT NULL THEN (wp.data->>'seedPlanting')::jsonb->>'action'
      ELSE NULL
    END
  ),
  seed_planting_custom = (
    CASE 
      WHEN (wp.data->>'seedPlanting') IS NOT NULL THEN (wp.data->>'seedPlanting')::jsonb->>'customAction'
      ELSE NULL
    END
  )
...
FROM mentoring_analytics ma
JOIN worksheet_progress wp ON ma.anonymous_id = wp.anonymous_id
WHERE (wp.data->>'seedPlanting') IS NOT NULL
